...
def hotelbooking(srno):
    if request.method == "POST":
        if "users" in session:
            ...
            booking = user_bookings(e_name=session['users'], service_name=hotel_name, rooms=no_rooms, check_in=check_in,
    ...
    if request.method == "POST":
        if "users" in session:
            ...
            booking = user_bookings(e_name=session['users'], service_name=homestay_name, check_in=check_in,
                                    days=days, total=total, type_of="homestay", from_des=from_des,
...
def cabbook(srno):
    if request.method == "POST":
        if "users" in session:
            ...
            booking = user_bookings(e_name=session['users'], service_name=cab_name, check_in=check_in,
                                    from_des=from_des, to_des=to_des, time=time,
...
def login():
    if 'user' in session and session['user'] == params["admin_user"]:
        ...
        return render_template("userdashboard.html", params=params, invalid="False", user_data=login_user,
                               details_wanted=bookings)
...
def uploadhomesstay(srno):
    if 'user' in session and session['user'] == params["admin_user"]:
        if request.method == "POST":
            ...
            if srno == "0":
                home_stay = homestayvillas(homestayname=hname, area=harea, price=cost, description=description,
                                           image_src=image_src, roomsoffered=rooms)
...
def uploadcabs(srno):
    if 'user' in session and session['user'] == params["admin_user"]:
        if request.method == "POST":
            ...
            if srno == "0":
                cab = tourcabs(cab_name=cab_name, cab_type=cab_type, price=price, image_src=image_src,
                               seats_available=seats_available, cab_description=cab_description)
                db.session.add(cab)
                db.session.commit()
            else:
...
def uploadtourpack(srno):
    if 'user' in session and session['user'] == params["admin_user"]:
        if request.method == "POST":
            ...
            if srno == "0":
                tour = tourpackages(tourname=tour_name, area=area, price=price, image_src=image_src, days=days,
